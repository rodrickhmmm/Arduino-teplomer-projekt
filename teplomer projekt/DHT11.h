...
typedef void (*PinModeFunc)(int pin, int mode);
typedef void (*DigitalWriteFunc)(int pin, int value);
typedef int  (*DigitalReadFunc)(int pin);
...
public:
     ...
     * @param pinModeFunc: Callback function for setting the pin mode (INPUT/OUTPUT).
     * @param digitalWriteFunc: Callback function for writing digital values (HIGH/LOW) to the pin.
     * @param digitalReadFunc: Callback function for reading digital values (HIGH/LOW) from the pin.
     * @param delayFunc: Callback function for implementing delay in milliseconds.
     */
    DHT11(
        int pin,
...
private:
    ...
    unsigned long _delayMS;       // Default delay in milliseconds between sensor readings.

    // Callbacky
    PinModeFunc _pinMode;
    DigitalWriteFunc _digitalWrite;
    DigitalReadFunc _digitalRead;
    DelayFunc _delay;
